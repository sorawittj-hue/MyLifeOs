  ...
  return (
    <div className="p-5 space-y-5 pb-28">
      <header className="flex justify-between items-center">
        <div>
          <p className={`text-[10px] font-semibold uppercase tracking-[0.2em] ${textMuted} mb-0.5`}>Your Health Hub</p>
      ...
      <motion.div
        ...
        className={`${cardBg} rounded-[2rem] p-6 text-center space-y-4 relative overflow-hidden`}
      >
        ...
        <div className="relative inline-block">
          {firebaseUser?.photoURL ? (
            ...
            <div className="w-20 h-20 bg-gradient-to-br from-green-400 to-green-600 rounded-2xl flex items-center justify-center text-black font-bold text-4xl shadow-lg shadow-green-500/20 mx-auto">
              {user?.name?.[0] || (user?.gender === 'male' ? 'M' : 'F')}
            </div>
          )}
          <div className={`absolute -bottom-2 -right-2 ${isDark ? 'bg-[#141414] border-[#141414]' : 'bg-white border-white'} border-4 rounded-full p-1 text-green-500`}>
            <Award size={20} />
          </div>
        </div>
        <div>
          ...
          <div className="flex items-center justify-center gap-2 mt-1">
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${isDark ? 'bg-white/[0.06] text-zinc-400' : 'bg-black/[0.04] text-zinc-500'}`}>
              {firebaseUser ? 'Online Member' : 'LifeOS Member'}
            </span>
            {firebaseUser && (
              <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${isDark ? 'bg-green-500/10 text-green-500' : 'bg-green-100 text-green-600'}`}>
                Synced
              </span>
            )}
          </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className={`${cardBg} bento-card p-4 flex items-center gap-4`}>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {latestVitals.map((vital) => (
              <div key={vital.id} className={`${cardBg} bento-card p-4 space-y-2`}>
      ...
      <section className="space-y-4">
        ...
        <div className={`${cardBg} bento-card p-5 space-y-4`}>
          <div className="flex items-center gap-4">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${firebaseUser ? 'bg-green-500/10 text-green-500' : (isDark ? 'bg-white/[0.06] text-zinc-500' : 'bg-black/[0.04] text-zinc-500')}`}>
              <Activity size={24} />
            </div>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-3">
          {[
            { title: 'ผู้เริ่มต้นสุขภาพดี', desc: 'เริ่มบันทึกข้อมูลสุขภาพครั้งแรก', icon: Activity, color: 'text-green-500', bg: 'bg-green-500/10', completed: todayNutrition.calories > 0 || latestVitals.length > 0 },
            ...
            <motion.div
              ...
              className={`${cardBg} bento-card p-4 flex items-center gap-4 ${item.completed ? '' : 'opacity-50'}`}
            >
              ...
              {item.completed ? (
                ...
                <div className={`w-6 h-6 border-2 ${isDark ? 'border-zinc-700' : 'border-zinc-300'} rounded-full`} />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        onClick={() => navigate('/settings')}
        className={`w-full py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 transition-colors bento-card ${
          isDark ? 'bg-white/[0.04] text-white hover:bg-white/[0.07]' : 'bg-black/[0.03] text-zinc-900 hover:bg-black/[0.05]'
        }`}
      >
